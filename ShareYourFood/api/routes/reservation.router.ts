...
reservationRouter.get("/:by", isAuth, async function(req,res) {
    const reservationController = await ReservationController.getInstance();
    const reservation = await reservationController.getReservationsByPlateId(req.params.by);
    if(reservation !== null) {
...
reservationRouter.get("/:plateId/user/:userId", isAuth, async function(req,res) {
    const reservationController = await ReservationController.getInstance();
    const reservation = await reservationController.getReservationsByPlateIdAndByUserId(req.params.plateId, req.params.userId);
    if(reservation !== null) {
        res.status(200).json(reservation).end();
    } else {
