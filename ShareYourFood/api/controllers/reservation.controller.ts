...
export class ReservationController {
    ...
    public async getReservationsByPlateId(name: any): Promise<ReservationInstance[] | null> {
        const id = parseInt(name);
        let reservation;
        if(isNaN(id)){
            return null; //TODO
        }else{
            reservation = await this.Reservation.findAll({
                attributes: ['id', 'date', 'received', 'UserId', 'PlateId'],
                where: { PlateId:id },
                include: [{ model: this.Plate, attributes: ['id', 'name', 'photo', 'quantity', 'comment'], 
    ...
    public async getReservationsByPlateIdAndByUserId(plateId: any, userId: any): Promise<ReservationInstance | null> {
        const pId = parseInt(plateId);
